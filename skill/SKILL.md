...
2. **PM archetype** — Growth, Core/Feature, Platform/Technical, AI/ML, Monetization, 0→1, or generalist. This decides which metrics to feature (see `pm-metrics-and-impact.md`).
...
Follow `references/pm-resume-structure.md` and the specific template spec. Build with the `docx` skill. Single column, ATS-safe font, no tables-for-layout, no icons/graphics, standard section names, right-aligned dates with en dashes.
...
- [ ] Metrics use digits and show baseline/scale ($600K, 18%, 14M users, 40→10 min)
...
- [ ] Dates right-aligned, en dashes, "Present" for current
